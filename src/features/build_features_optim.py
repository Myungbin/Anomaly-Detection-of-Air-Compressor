...
def create_derived_features(df):
    ...
    #  df['motor_power_factor'] = np.cos(np.arccos(df['power_efficiency']) - np.arctan(df['motor_vibe'] / df['motor_current']))
    #  df['motor_power_density'] = df['motor_power'] / (df['air_flow_rate'] * df['air_density'])
    #  df['specific_work'] = df['out_pressure'] * 1000 / df['air_density'] - 101325
    #  df['temperature_ratio'] = df['air_end_temp'] / df['motor_temp']
    #  df['pressure_ratio'] = df['out_pressure'] / (101325 * df['motor_temp'] / df['air_end_temp'])
    ...
    # df['air_moles'] = df['out_pressure'] * 10**6 / (8.31 * (df['air_end_temp'] + 273))
    # df['air_quality'] = (df['air_inflow'] * 60) / (1000 * df['air_moles'])
    # df['electric_power'] = df['motor_current'] * 380 * np.sqrt(3) / 1000
    # df['volumetric_efficiency'] = df['air_flow_rate'] / (df['motor_rpm'] * 1000 / 60) * 100
    # df['thermal_efficiency'] = (df['air_inflow'] * 60 * 1.4 * 287 * (df['air_end_temp'] + 273)) / (df['motor_power'] * 1000)
    # df['motor_torque'] = (9.55 * df['motor_power'] * 1000) / df['motor_rpm']
    # df['motor_slip'] = (df['motor_rpm'] - (df['out_pressure'] * 10 / 2.45) * 60 / (2 * np.pi * 50)) / df['motor_rpm'] * 100
    # df['motor_frequency'] = df['motor_rpm'] / 60 * 50
    # df['motor_impedance'] = (380 / df['motor_current']) / np.sqrt(3)
    # df['motor_pf'] = df['power_efficiency'] / np.sqrt(df['power_efficiency']**2 + (df['motor_power_factor']**2 - df['power_efficiency']**2)**0.5)
    # df['motor_ea'] = (df['motor_current'] / df['motor_impedance']) * np.sin(np.arccos(df['motor_pf']))
    # df['motor_torque_factor'] = (df['motor_torque'] / (df['motor_current'] * df['motor_impedance'])) * np.sin(np.arccos(df['motor_pf']))
    # df['motor_efficiency'] = df['motor_power'] / df['electric_power'] * 100

    # df['air_specific_heat'] = 1.005 + (df['air_end_temp'] - 25) * 0.00114
    # df['delta_enthalpy'] = df['air_inflow'] * 60 * df['air_specific_heat'] * (df['out_pressure'] * 10 / 2.45 - 1.01325) / 1000
    # df['air_velocity'] = df['air_inflow'] / (np.pi * (0.15**2) / 4)
    # df['fan_laws_4'] = df['air_flow_rate'] * (df['motor_temp'] + 273) / (df['air_end_temp'] + 273)
    # df['fan_laws_5'] = df['air_inflow'] * (df['motor_temp'] + 273) / (df['air_end_temp'] + 273)
    # df['fan_laws_6'] = df['out_pressure'] * (df['motor_temp'] + 273) / (df['air_end_temp'] + 273)
    # df['fan_laws_7'] = df['air_flow_rate'] * (df['out_pressure'] / (101325 * df['motor_temp'] / df['air_end_temp'])) ** 0.5
    return df
